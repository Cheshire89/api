...
const debug = util.debuglog('ValidateChecksWorker');
...
class ValidateChecksWorker{
    ...
    validateCheckData(checkData) {
        ...
        if(validator.valid){
            ...
            debug(`Error: ${checkData.id} fields are not properly formated [${validator.fields.toString()}]`)
        }
    };
        ...
        _data.update('checks', newCheckData.id, newCheckData, err => {
            if(!err){
                if(alertWaranted){
                    ...
                    debug(`${newCheckData.id}: Check outcome has not changed, no alert needed`);
                }
            } else {
                debug(`Error: [${newCheckData.id}] check failed to update`);
            }
        })
    };
    ...
    alertUserToStatusChange(checkData) {
        ...
        helpers.sendTwillioSms(checkData.userPhone, message, (err, callback) => {
            if(!err){
                debug('User was allrted to a status change in their check via sms!', message);
            } else {
                debug(`Error: Could not send sms alert to [${checkData.userPhone}]`);
            }
        })
    };
